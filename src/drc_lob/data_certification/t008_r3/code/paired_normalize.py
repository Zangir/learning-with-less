"""Acquisition-bound paired Tardis snapshots, with explicit offline clock limits.
...
def normalize_lines(lines, date, start_ns, end_ns, max_gap_ns):
    ...
    for asset in rows:
        if rows[asset]:
            segments[asset].append((segment_first[asset], len(rows[asset])))
    return rows, provenance, duplicates, segments, disconnects, excluded
...
def build_day(records, date, roles, policy, output, start_ns=None, end_ns=None, policy_path=None):
    ...
    if policy_path is not None:
        verify_policy_binding(records, date, roles, policy, P.sha256(policy_path), start_ns, end_ns)
    proofs = [verify_acquisition(record) for record in records]
    raw_rows, provenance, duplicates, segments, disconnects, excluded = normalize_lines(record_lines(records), date, start_ns, end_ns, policy["max_gap_ns"])
    ...
    for asset in ("BTC", "ETH"):
        ...
        write_json(target / "shared_data_contract.v2.2.0.json", contract)
        summary = {"asset": asset, "date": date, "split_roles": roles, "rows": len(rows), "raw_in_period_rows": len(provenance[asset]),
            ...
            "contract_file": str(target / "shared_data_contract.v2.2.0.json"), "contract_sha256": P.sha256(target / "shared_data_contract.v2.2.0.json"),
    ...
    result = {"date": date, "split_roles": roles, "assets": summaries, "paired_common_cuts": pairing,
        ...
        "disconnect_markers": len(disconnects), "elapsed_seconds": time.perf_counter()-started, "model_fits": 0}
...
def verify_output(asset_output):
    ...
    contract_path = target / "shared_data_contract.v2.2.0.json"
...
def emit_schema(output):
    ...
    schema["title"] = "T-008 paired sampled snapshot producer contract2.2.0"
    schema["properties"]["version"] = {"const": "2.2.0"}
    ...
    write_json(Path(output) / "shared_data_contract.v2.2.0.schema.json", schema)
    return schema
