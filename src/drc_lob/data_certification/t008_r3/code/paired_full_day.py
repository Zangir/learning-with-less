...
def main():
    ...
    while len(completed) < len(policy["roles_Q17"]):
        for date in policy["roles_Q17"]:
            ...
            if index["date"] != date or index["protocol_sha256"] != N.P.sha256(policy_path) or len(index["records"]) != 144:
        ...
        if len(completed) < len(policy["roles_Q17"]):
            time.sleep(10)
    result = {"schema":"t008-paired-dataset-index/3","version":"2.2.0","origin":"exploratory_real","stage":"full_day",
        ...
        "source_scope":"Third-party archived full snapshots; fixed full UTC day acquisition, strict event-order policy; independent review pending"}
    index_path = out / "dataset_index.v2.2.0.json"
    N.write_json(index_path,result)
    output = bind_index(index_path)
