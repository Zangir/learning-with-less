...
def main():
    ...
    for kind, source in [('after', after), ('before', before)]:
        for row in source:
            ...
            stats[kind+'_inside_level_candidate_group'] += not level_terminal
    ...
    (OUT/'q16_actual_cut_coverage.jsonl').write_text(''.join(json.dumps(r,separators=(',',':'))+'\n' for r in details))
    summary = dict(summary=dict(stats), origin='exploratory_real', admitted=False,
        ...
        output_sha256=sha(OUT/'q16_actual_cut_coverage.jsonl'), code_sha256=sha(Path(__file__)))
    (OUT/'q16_actual_cut_summary.json').write_text(json.dumps(summary,indent=2)+'\n')
