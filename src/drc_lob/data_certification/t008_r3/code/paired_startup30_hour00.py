...
def main():
    ...
    if index["protocol_sha256"] != policy["supersedes_policy_sha256"] or index["completed_count"] != 72:
    ...
    out = N.ROOT / "interface/hour00_startup30"
    if out.exists():
    ...
    for date in policy["roles_Q17"]:
        ...
        try:
            if (out / date / "paired_summary.json").exists():
                ...
                result = N.build_day(selected, date, roles, policy, out / date,
                    start_ns=N.utc_ns(date+"T00:00:00Z")+30*N.NS, policy_path=policy_path)
    ...
    result = {"schema":"t008-paired-dataset-index/3", "version":"2.3.0", "origin":"exploratory_real", "stage":"hour00_startup30",
        ...
        "source_scope":"Third-party archived full snapshots; source-quality-informed post-acquisition pre-fit fixed30s adaptation; exact-input RV011 review pending",
    ...
    N.write_json(out / "dataset_index.v2.3.0.json", result)
    print(json.dumps({"status":"complete", "index_sha256":N.P.sha256(out / "dataset_index.v2.3.0.json"),"elapsed_seconds":result["elapsed_seconds"]}), flush=True)
