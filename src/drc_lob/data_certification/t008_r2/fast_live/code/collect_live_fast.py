...
def status(state):
    out={"origin":"exploratory_real","route":"official_fast_live_snapshot_pilot","status":state,
      ...
      "semantics":"sampled authoritative5-level fast snapshots and block BBO changes; no venue-event completeness claimed",
...
with gzip.open(OUT,"xt",encoding="utf-8",newline="\n",compresslevel=3) as out:
    while time.monotonic()-START<DURATION_SECONDS and connections<4:
        ...
        try:
            ...
            for coin in ["BTC","ETH"]:
                send(ws,{"method":"subscribe","subscription":{"type":"l2Book","coin":coin,"fast":True}})
