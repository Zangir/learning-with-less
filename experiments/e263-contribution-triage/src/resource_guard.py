...
class Accounting(ctypes.Structure):
    _fields_ = [(name,ctypes.c_int64) for name in ('User','Kernel','PeriodUser','PeriodKernel')]+[(name,w.DWORD) for name in ('PageFaults','TotalProcesses','ActiveProcesses','TerminatedProcesses')]
...
def record(name, extra=None):
    ...
    assert k.QueryInformationJobObject(job,1,ctypes.byref(accounting),ctypes.sizeof(accounting),None)
    metadata={'runtime_seconds':time.perf_counter()-START,'cpu_seconds':(accounting.User+accounting.Kernel)/1e7,'cpu_affinity':[8,9],'job_memory_limit_bytes':limits.JobMemory,'peak_job_memory_bytes':limits.PeakJob,'python':sys.version,'processes_in_job':accounting.TotalProcesses}
